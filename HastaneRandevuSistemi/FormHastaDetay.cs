...
namespace HastaneRandevuSistemi
{
    public partial class FormHastaDetay : Form
    {
        ...
        void AktifRandevulariListele()
        {
            ...
            try
            {
                // Aktif Randevuları getiren sorgu
                string sorgu = @"
                    SELECT 
                        r.randevu_id AS ""ID"",
        ...
        private void CmbSehir_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                string sorgu = "SELECT hastane_ad FROM Hastaneler WHERE sehir = (@p1) ORDER BY hastane_ad";
                NpgsqlCommand command = new NpgsqlCommand(sorgu, conn);
                command.Parameters.AddWithValue("@p1", CmbSehir.Text.Trim());
        ...
        private void CmbHastAd_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                string sorgu = "SELECT DISTINCT b.brans_ad FROM Branslar b JOIN Hastaneler h ON b.hastane_id = h.hastane_id WHERE h.hastane_ad=@p1 ORDER BY b.brans_ad";
                NpgsqlCommand command = new NpgsqlCommand(sorgu, conn);
                command.Parameters.AddWithValue("@p1", CmbHastAd.Text);
                NpgsqlDataReader dr = command.ExecuteReader();
        ...
        private void CmbBrans_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                string sorgu = "SELECT (d.doktor_ad || ' ' || d.doktor_soyad) FROM Doktorlar d JOIN Branslar b ON d.brans_id = b.brans_id JOIN Hastaneler h ON d.hastane_id = h.hastane_id WHERE h.hastane_ad=@p1 AND b.brans_ad=@p2";
                NpgsqlCommand command = new NpgsqlCommand(sorgu, conn);
                command.Parameters.AddWithValue("@p1", CmbHastAd.Text);
                command.Parameters.AddWithValue("@p2", CmbBrans.Text);
                NpgsqlDataReader dr = command.ExecuteReader();
        ...
        private void BtnRecete_Click(object sender, EventArgs e)
        {
            ...
            {
                string tcHash = SecurityHelper.Hashle(TC);
                NpgsqlCommand command = new NpgsqlCommand("SELECT hasta_id FROM Hastalar WHERE tc_hash = @p1", conn);
                command.Parameters.AddWithValue("@p1", tcHash);
                NpgsqlDataReader dr = command.ExecuteReader();
                if (dr.Read())
                {
                    string hastaID = dr[0].ToString();
                    ReceteGoruntule frm = new ReceteGoruntule();
                    frm.hasta_ID = hastaID;
                    frm.Show();
                }
