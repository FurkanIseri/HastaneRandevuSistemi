...
namespace HastaneRandevuSistemi
{
    public partial class FormSekreterGiris : Form
    {
        ...
        private void BtnLogIn_Click(object sender, EventArgs e)
        {
            ...
            NpgsqlCommand command = new NpgsqlCommand("SELECT * FROM Sekreterler WHERE tc_hash=@par1 AND sifre_hash=@par2", conn);
        ...
        private void SifremiUnuttum_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            string tcHash = SecurityHelper.Hashle(MskTxtTC.Text);
            NpgsqlConnection conn = bgl.baglanti();
            NpgsqlCommand command2 = new NpgsqlCommand("SELECT sifre_sifreli FROM Sekreterler WHERE tc_hash = @par1", conn);
            command2.Parameters.AddWithValue("@par1", tcHash);
            object sonuc = command2.ExecuteScalar();
            if (sonuc != null)
            {
                // Şifreyi hatırlamak için.
            ...
            {
                MessageBox.Show("Böyle bir sekreter kaydı bulunmamaktadır.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            conn.Close();
        }
        private void FormSekreterGiris_FormClosing(object sender, FormClosingEventArgs e)
        {
